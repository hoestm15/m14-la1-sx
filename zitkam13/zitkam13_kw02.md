...
## Exceptions in JAVA  
Die Klasse **Exception** wird von **Throwable** abgeleitet. Diese wiederum wird vom der Klasse Object abgelditet. Bei schweren Fehlern wir in der **JVM(Java Virtual Machine)** ein Error erzeugt, welcher zu einem Absturz des Programms führt. Alle Errors die von **JSSC (Java serielle Schnittstelle)** geworfen werden, ,üssen weitergeworfen werden, wenn nur Exceptions abgefangen werden. Für die Lösung dieses Problems gibt es zwei Lösungen. Entweder wird ein **Multicatch** verwendet oder es werden alle **Throwables** abgefangen. Mit beiden Varianten können **Errors** und **Exceptions** behandelt werden. 
